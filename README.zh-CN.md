...
| 静态对象构造/析构 | ✅ | 驱动加载时自动执行 |
| SAFESEH / GS | ✅ | 缓冲区安全检查 |
| SEH（`__try`/`__except`） | ✅ | 访问违规、finally、continue search |
| STL 容器/算法 | ✅ | 完整 OneCore / CoreCRT 支持 |
| `std::format` / `std::print` | ✅ | C++23 格式化输出 |
| `std::regex` | ✅ | 完整正则表达式支持 |
| `std::filesystem` | ✅ | C++17 文件系统（阶段一） |
| 字符串转换（`std::stoi`/`stod` 等） | ✅ | stoi, stol, stoul, stoll, stoull, stod, stof |
