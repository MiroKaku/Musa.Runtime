    ...
    )
{
    ...
    char    *szLineMessage = nullptr;
    char    *szOutMessage  = nullptr;
    wchar_t *szOutMessage2 = nullptr;
    char    *szUserMessage = nullptr;
#endif
    ...
    {
    #if defined _UCRT_ENCLAVE_BUILD
        szLineMessage = static_cast<char*   >(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(char)));
        szOutMessage  = static_cast<char*   >(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(char)));
        szOutMessage2 = static_cast<wchar_t*>(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(wchar_t)));
        szUserMessage = static_cast<char*   >(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(char)));
        if (szLineMessage == nullptr ||
            szOutMessage  == nullptr ||
            szOutMessage2 == nullptr ||
            szUserMessage == nullptr) {
    ...
    #if defined _UCRT_ENCLAVE_BUILD
        if (szLineMessage) {
            LocalFree(szLineMessage);
        }
        if (szOutMessage) {
            LocalFree(szOutMessage);
        }
        if (szOutMessage2) {
            LocalFree(szOutMessage2);
        }
        if (szUserMessage) {
            LocalFree(szUserMessage);
        }
    #endif
    }
    ...
    )
{
    ...
    wchar_t *szLineMessage = nullptr;
    wchar_t *szOutMessage  = nullptr;
    char    *szOutMessage2 = nullptr;
    wchar_t *szUserMessage = nullptr;
#endif
    ...
    {
    #if defined _UCRT_ENCLAVE_BUILD
        szLineMessage = static_cast<wchar_t*>(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(wchar_t)));
        szOutMessage  = static_cast<wchar_t*>(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(wchar_t)));
        szOutMessage2 = static_cast<char*   >(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(char)));
        szUserMessage = static_cast<wchar_t*>(LocalAlloc(LPTR, DBGRPT_MAX_MSG * sizeof(wchar_t)));
        if (szLineMessage == nullptr ||
            szOutMessage  == nullptr ||
            szOutMessage2 == nullptr ||
            szUserMessage == nullptr) {
        ...
        if (szFormat)
        {
            ...
            size_t const max_user_message_count = DBGRPT_MAX_MSG - 2 - max_assert_intro_count;
            ...
            // well.
            int szlen = 0;
            _ERRCHECK_SPRINTF(szlen = _vsnwprintf_s(
                szUserMessage,
                DBGRPT_MAX_MSG,
