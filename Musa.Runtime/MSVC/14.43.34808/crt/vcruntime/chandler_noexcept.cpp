...
// __GSHandlerCheck_SEH_noexcept — GS cookie check for noexcept functions
// Generated by /GS compiler flag when compiling noexcept functions (open.obj, fopen.obj, etc.)
#if defined _M_AMD64 || defined _M_ARM || defined _M_ARM64
...
extern "C" EXCEPTION_DISPOSITION __cdecl __GSHandlerCheck_SEH_noexcept(
    _In_    struct _EXCEPTION_RECORD* ExceptionRecord,
    _In_    void* EstablisherFrame,
    _Inout_ struct _CONTEXT* ContextRecord,
    _Inout_ struct _DISPATCHER_CONTEXT* DispatcherContext
...
{
    auto const Disposition = __GSHandlerCheck_SEH(ExceptionRecord, EstablisherFrame, ContextRecord, DispatcherContext);
    if (IS_DISPATCHING(ExceptionRecord->ExceptionFlags)
...
extern "C" DECLSPEC_GUARD_SUPPRESS EXCEPTION_DISPOSITION __cdecl __GSHandlerCheck_noexcept(
    _In_ PEXCEPTION_RECORD              ExceptionRecord,
    _In_ PEXCEPTION_REGISTRATION_RECORD EstablisherFrame,
    _Inout_ PCONTEXT                   ContextRecord,
    _Inout_ PVOID                      DispatcherContext
...
{
    auto const Disposition = __GSHandlerCheck(ExceptionRecord, EstablisherFrame, ContextRecord, DispatcherContext);
    if (IS_DISPATCHING(ExceptionRecord->ExceptionFlags)
        && ExceptionRecord->ExceptionCode == STATUS_STACK_BUFFER_OVERRUN
        && Disposition == ExceptionContinueSearch)
    {
