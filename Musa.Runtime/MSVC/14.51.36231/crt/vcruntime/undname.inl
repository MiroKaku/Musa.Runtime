...
public:
    ...
    template<StringLife life>
    pcharNode(pcchar_t, int, StringLifeSelector<life>, _HeapManager&);
...
inline void* operator new (size_t sz, _HeapManager& heap, int noBuffer)
{
    if (noBuffer)
...
void* _HeapManager::getMemoryWithBuffer(size_t sz)
{
    ...
    if (blockLeft < sz) {
        ...
        Block* pNewBlock = new(*this, 1) Block;
...
{
    return DName(rd.undecorator, c) + rd;
}

inline DName operator + (DNameStatus st, const DName& rd)
{
    return DName(rd.undecorator, st) + rd;
}

inline DName operator + (const StringLiteral& s, const DName& rd)
{
    return DName(rd.undecorator, s) + rd;
}


// The 'DName' constructors

inline DName::DName (UnDecorator* undecorator)
    : node(nullptr)
...
inline DName::DName (const DName & rd )
    : node(rd.node)
    , undecorator(rd.undecorator)
    , value(rd.value)
{
} // End of "DName" CONSTRUCTOR '(const DName&)'


DName::DName (UnDecorator* undecorator, char c )
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
...
DName::DName(UnDecorator* undecorator, const StringLiteral& s)
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
...
template<StringLife life>
DName::DName(UnDecorator* undecorator, pcchar_t name, StringLifeSelector<life>)
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
...
DName::DName (UnDecorator* undecorator, pcchar_t & name, char terminator )
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
...
DName::DName(UnDecorator* undecorator, DNameStatus st)
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
...
DName::DName(UnDecorator* undecorator, DName* pd)
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
    if (pd) {
        node = gnew2 pDNameNode(pd);
...
DName::DName(UnDecorator* undecorator, unsigned __int64 num)
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
...
DName::DName(UnDecorator* undecorator, __int64 num)
    : node(0)
    , undecorator(undecorator)
    , value(0)
{
...
inline   void DName::value_object(bool val)
{
    value_object_ = val;
}

inline   bool DName::value_object() const
{
...
inline pchar_t DName::getString(_Inout_z_cap_(max) pchar_t buf, int max) const
{
    if (!isEmpty()) {
        ...
        if (!buf) {
            max = length() + 1;
            buf = gnew2 char[max]; // Get a buffer big enough
...
{
    if (newRight) {
        node = gnew2 pairNode(node, newRight);
        if (!node) {
            stat = DN_error;
        }
    }
    ...
    if (isValid() && str.len > 0) {
        if (isEmpty())
            *this = str;
            ...
            append(gnew2 pcharNode(str.str, str.len, StringLifeSelector<StringLife::Persistent>()));
        }
    }
    // And return self
...
void DName::doPchar(pcchar_t str, int len)
{
    ...
    elif(str && len > 0)
    {
        ...
        if (len == 1) {
            node = gnew2 charNode(*str);
            if (!node) {
                stat = DN_error;
            }
        }
        else {
            node = gnew2 pcharNode(str, len, StringLifeSelector<life>());
            if (!node) {
                stat = DN_error;
            }
        }
...
inline     Replicator::Replicator(UnDecorator* undecorator)
    : index(-1)
    , undecorator(undecorator)
{
#ifdef UND_STATS
...
{
    if ((x < 0) || (x > 9))
        return DName(undecorator, DN_error);
    elif((index == -1) || (x > index))
        return DName(undecorator, DN_invalid);
    else
...
template<StringLife life>
pcharNode::pcharNode(pcchar_t str, int len, StringLifeSelector<life>, _HeapManager& heap)
{
    if constexpr (life == StringLife::Persistent) {
        me = const_cast<pchar_t>(str);
        myLen = len;
