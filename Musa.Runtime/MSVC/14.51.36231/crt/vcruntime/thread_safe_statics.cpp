...
// be modified while the lock is held.  _Init_thread_epoch is readable from user
...
// of this feature the event is sufficient but not optimal.  See the code in _Init_thread_wait_v2
// below.
//
...
{
    // Always succeeds on Vista and later.
    (void) InitializeCriticalSectionAndSpinCount(&g_tss_mutex, 4000);
...
{
#if _USE_VISTA_THREAD_SAFE_STATICS
    SleepConditionVariableSRW(&g_tss_cv, &g_tss_srw, INFINITE, 0);
#else // ^^^ _USE_VISTA_THREAD_SAFE_STATICS ^^^ // vvv !_USE_VISTA_THREAD_SAFE_STATICS vvv //
    if (g_sleep_condition_variable_cs)
    {
        // Vista+ code is first because it's most likely
        g_sleep_condition_variable_cs(&g_tss_cv, &g_tss_mutex, INFINITE);
        return;
    }

    // In the XP implementation using only a Windows event
    ...
    _Init_thread_unlock();
    WaitForSingleObjectEx(g_tss_event, 100 /* milliseconds */, FALSE);
    _Init_thread_lock();
#endif // _USE_VISTA_THREAD_SAFE_STATICS
}
...
extern "C" void __cdecl _Init_thread_header(int* const ptrOnce) noexcept
{
    _Init_thread_lock();

    if (*ptrOnce == uninitialized)
    {
        *ptrOnce = being_initialized;
    }
    else
    {
        while (*ptrOnce == being_initialized)
        {
...
extern "C" void __cdecl _Init_thread_abort(int* const ptrOnce) noexcept
{
    _Init_thread_lock();
    *ptrOnce = uninitialized;
    _Init_thread_unlock();
    _Init_thread_notify();
}
...
extern "C" void __cdecl _Init_thread_footer(int* const ptrOnce) noexcept
{
    _Init_thread_lock();
    ++_Init_global_epoch;
