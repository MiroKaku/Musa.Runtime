...
public:
	...
	// Get/Set whether this name represents a value object: i.e., a literal class type
	void value_object(bool value);
	bool value_object() const;
...
#pragma warning(disable: 4201) // nonstandard extension used: nameless struct/union
		struct
		{
			...
			uint32_t isIndir       : 1;
			uint32_t isAUDC        : 1;
			uint32_t isAUDTThunk   : 1;
			uint32_t isArrayType   : 1;
			uint32_t NoTE          : 1;
			uint32_t pinPtr        : 1;
			uint32_t comArray      : 1;
			uint32_t vcallThunk    : 1;
			uint32_t value_object_ : 1;
		};
#pragma warning(pop)
	};
...
private:
	...
	// Avoid std::optional, which would be a layering violation because undname.cxx is used by VCRuntime.
	struct OptionalValue
	{
		...
		OptionalValue(std::uint64_t value) : m_hasValue{true}, m_value{value} {}
		explicit operator bool() const { return m_hasValue; }
		std::uint64_t operator*() const { return m_value; }
	};
...
DName UnDecorator::getDecoratedName(void)
{
	...
	else if(*gName == '?')
	{
		...
		if ((*gName == '\0') || (*gName == '@'))
		{
			...
			// If this is a value object then we just have a name
			if (symbolName.value_object())
			{
				return symbolName;
			}
...
UnDecorator::OptionalValue UnDecorator::getValue()
{
...
DName UnDecorator::getStringObject()
{
	...
	if (strncmp(gName, "??_C", strlen("??_C")) == 0)
	{
		increment_buffer_no_check(strlen("??_C"));
