...
#if !defined(_CPPCLI_IMPL_DETAILS)

// Specially named namespace containing implementation types and code
#define _CPPCLI_IMPL_DETAILS                     \
    __pragma(warning(push))                     \
    __pragma(warning(disable : 4483))           \
    __identifier("<CrtImplementationDetails>")  \
    __pragma(warning(pop))

#endif // ^^^ !defined(_CPPCLI_IMPL_DETAILS)

namespace _CPPCLI_IMPL_DETAILS
{
...
_CRT_SERIALIZABLE_ATTRIBUTE_FOR_CROSS_APPDOMAIN_USE
...
internal:
    ...
    void AddHandler(EventHandler^ handler)
    {
        ...
        _CRT_PREPARE_CONSTRAINED_REGIONS();
        try
        {
            _CRT_PREPARE_CONSTRAINED_REGIONS();
...
private:
    ...
    _CRT_CONSTRAINED_EXECUTION_PRE_PREPARE_METHOD
    _CRT_SECURITYCRITICAL_ATTRIBUTE
...
{
    // Execution in the default app-domain for NetCore. Since there is only
    // one app-domain, we simply call the function pointer with the given argument.
    (*function)(cookie);
}

inline AppDomain^ GetDefaultDomain()
{
    return AppDomain::CurrentDomain;    // NetCore has support for only one app-domain.
}

} // namespace _CPPCLI_IMPL_DETAILS

#else // ^^^ defined(_CLR_NETCORE) / !defined(_CLR_NETCORE) vvv
    ...
    try
    {
        ...
        pHost = reinterpret_cast<ICorRuntimeHost*>(
            RuntimeEnvironment::GetRuntimeInterfaceAsIntPtr(
                _CPPCLI_IMPL_DETAILS::FromGUID(__uuidof(CorRuntimeHost)),
                _CPPCLI_IMPL_DETAILS::FromGUID(__uuidof(ICorRuntimeHost))).ToPointer());
    }
    catch (Exception^ ex)
    {
...
inline void DoCallBackInDefaultDomain(MixedFunctionType function, void* cookie)
{
    ...
    pClrHost = reinterpret_cast<ICLRRuntimeHost*>(
        RuntimeEnvironment::GetRuntimeInterfaceAsIntPtr(
            _CPPCLI_IMPL_DETAILS::FromGUID(__uuidof(CLRRuntimeHost)),
            _CPPCLI_IMPL_DETAILS::FromGUID(__uuidof(ICLRRuntimeHost))).ToPointer());
