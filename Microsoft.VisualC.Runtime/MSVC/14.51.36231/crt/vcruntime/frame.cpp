...
////////////////////////////////////////////////////////////////////////////////
//
// x64/arm64-specific definitions
//
...
EXCEPTION_DISPOSITION __InternalCxxFrameHandlerWrapper(
    ...
    if constexpr (std::is_same_v<T, RENAME_EH_EXTERN(__FrameHandler4)>)
    {
        ...
        __try
        {
            retVal = __InternalCxxFrameHandler<T>(pExcept, pRN, pContext, pDC, pFuncInfo, CatchDepth, pMarkerRN, recursive);
        }
        __finally
        {
#if _VCRT_BUILD_FH4
...
EXCEPTION_DISPOSITION __InternalCxxFrameHandler(
    ...
    if ((cxxReThrow == false) && (PER_CODE(pExcept) != EH_EXCEPTION_NUMBER) &&
        ...
        /* On the 64 bit/ARM64 platforms, ExceptionCode maybe set to STATUS_UNWIND_CONSOLIDATE
...
* Exception was handled and __GetCurrentState for retrieving the current
...
* }
*
* If we have an exception coming from STATE3, the FindHandler will be
...
*
* Also look at code in __CxxCallCatchBlock, you will see that as soon as we get
                ...
                __try {
                    ...
                    if ((UWEntry.type == UnwindMapEntry4::Type::DtorWithObj) || (UWEntry.type == UnwindMapEntry4::Type::DtorWithPtrToObj))
                    {
                        ...
                        RENAME_EH_EXTERN(_CallSettingFrameEncoded)((void*)__RVAtoRealOffset(pDC, dispAction),
                            *pRN,
                                objectAddr,
#if defined(_M_ARM64) || defined(_CHPE_X86_ARM64_EH_)
                            (PULONG)pDC->NonVolatileRegisters,
#endif // defined(_M_ARM64) || defined(_CHPE_X86_ARM64_EH_)
                            0x103);
                    }
                    else
...
static void CatchIt(
    ...
    // unless the catch was by ellipsis (no conversion) OR the catch was by
    ...
    T::SetState(pRN, pDC, pFuncInfo, TBME_HIGH(*pEntry) + 1);
    ...
    )
{
    ...
    __updateAdjForSve(pFuncInfo, pContext, fixed_adj, sve_adj);
#endif
    UNWINDHELP(*pEstablisherFrame, fixed_adj, sve_adj) = -2;
#endif
    return continuationAddress;
}
