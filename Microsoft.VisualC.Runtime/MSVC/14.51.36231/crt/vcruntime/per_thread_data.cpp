...
extern "C" void __cdecl __vcrt_freeptd_for_this_thread()
{
    // Do nothing unless per-thread data has been allocated for this module:
    if (__vcrt_flsindex == FLS_OUT_OF_INDEXES)
        return;

    // Get the pointer for this thread.  Note that we
    // must not call __vcrt_getptd, because it will allocate a new per-thread
    // data if one does not already exist.
    __vcrt_ptd* const block_to_free = static_cast<__vcrt_ptd*>(__vcrt_FlsGetValue(__vcrt_flsindex));
...
extern "C" void WINAPI __vcrt_freefls(_Pre_maybenull_ _Post_invalid_ void* const pfd)
{
