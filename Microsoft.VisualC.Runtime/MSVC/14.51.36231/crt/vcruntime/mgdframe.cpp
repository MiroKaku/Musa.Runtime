    ...
    )
{
    ...
    if (PER_IS_MSVC_EH(pExcept))
    {
        if ( PER_PTHROW(pExcept) == nullptr) {
            ...
            /*
            * After reading _pCurrentException, verify pExcept is still a C++ exception.
            * With .NET Framework 4.8+, _pCurrentException may contain a .NET exception
            * (code 0xe0434352) rather than a C++ exception (code 0xe06d7363).
            * Attempting to access C++ exception metadata on a .NET exception causes
            * a null pointer dereference crash, so return and let the CLR handle it.
            */
            if (!PER_IS_MSVC_EH(pExcept)) {
                return EXCEPTION_CONTINUE_SEARCH;
            }
...
#if _EH_RELATIVE_FUNCINFO
            void *CatchImageBase = nullptr;
            // Set the image base using Rtl API.
            if (pType) {
                CatchImageBase = RtlPcToFileHeader(pType, &CatchImageBase);
            }
