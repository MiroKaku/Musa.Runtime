...
namespace Main
{
    ...
    static VOID NTAPI TestThreadRoutine(_In_ PVOID)
    {
        ...
        KeExpandKernelStackAndCalloutEx([](PVOID)
        {
            ...
            {
                std::vector<int, kallocator<int, NonPagedPool, 'TsrT'>> vec;
            ...
            {
                std::vector<int, kallocator<int>> vec = {3, 1, 4, 1, 5};
                vec.shrink_to_fit();
            ...
            {
                std::vector<int, kallocator<int>> src = {100, 200, 300};
                auto dst = src;
            ...
            // Map
            {
                using MT = std::map<int, std::string, std::less<int>, kallocator<std::pair<const int, std::string>>>;
                MT m;
            ...
            {
                using MT = std::map<std::string, int, std::less<std::string>, kallocator<std::pair<const std::string, int>>>;
                MT m;
            ...
            {
                using MT = std::map<int, int, std::less<int>, kallocator<std::pair<const int, int>>>;
                MT m;
            ...
            // Unordered map
            {
                using MT = std::unordered_map<int, std::string, std::hash<int>, std::equal_to<int>, kallocator<std::pair<const int, std::string>>>;
                MT m;
            ...
            // Set
            {
                std::set<int, std::less<int>, kallocator<int>> s;
            ...
            // Unordered set
            {
                std::unordered_set<int, std::hash<int>, std::equal_to<int>, kallocator<int>> s;
            ...
            // List
            {
                std::list<int, kallocator<int>> lst;
            ...
            {
                std::list<int, kallocator<int>> lst = {5, 3, 1, 4, 2};
                lst.sort();
                auto it = lst.begin();
            ...
            {
                std::list<int, kallocator<int>> a = {1, 2, 3}, b = {4, 5};
                a.merge(b);
                KTEST_EXPECT(a.size() == 5 && b.empty(), "List_Merge");
            }
            {
                std::list<int, kallocator<int>> lst2 = {1, 1, 2, 2, 3};
                lst2.unique();
            ...
            // Queue
            {
                std::queue<int, std::deque<int, kallocator<int>>> q;
            ...
            // Stack
            {
                std::stack<int, std::vector<int, kallocator<int>>> s;
            ...
            // Priority queue
            {
                std::priority_queue<int, std::vector<int, kallocator<int>>> pq;
            ...
            // Edge cases
            {
                std::vector<int, kallocator<int>> v;
                std::map<int, int, std::less<int>, kallocator<std::pair<const int, int>>> m;
                std::string s;
                KTEST_EXPECT(v.empty() && m.empty() && s.empty(), "Edge_AllEmpty");
                KTEST_EXPECT(v.begin() == v.end(), "Edge_BeginEndEqual");
            }
            {
                std::vector<int, kallocator<int>> v = {42};
                KTEST_EXPECT(v.size() == 1 && v.front() == 42 && v.back() == 42, "Edge_SingleElementVector");
            }

            // Algorithms: sort
            {
                std::vector<int, kallocator<int>> vec = {5, 3, 8, 1, 9, 2, 7, 4, 6, 0};
            ...
            {
                std::vector<int, kallocator<int>> vec = {3, 1, 4, 1, 5, 9, 2, 6};
                std::sort(vec.begin(), vec.end());
                bool ok = true;
            ...
            // Algorithms: find
            {
                std::vector<int, kallocator<int>> vec = {10, 20, 30, 40, 50};
            ...
            {
                std::vector<int, kallocator<int>> src = {1, 2, 3, 4, 5}, dst(5);
            ...
            {
                std::vector<int, kallocator<int>> src = {100, 200, 300}, dst(3);
                std::copy(src.begin(), src.end(), dst.begin());
                KTEST_EXPECT(dst[0] == 100 && dst[1] == 200 && dst[2] == 300, "Algorithm_Copy");
            }
            {
                std::vector<int, kallocator<int>> vec(5);
            ...
            // Algorithms: remove
            {
                std::vector<int, kallocator<int>> vec = {1, 2, 3, 2, 4, 2, 5};
            ...
            {
                std::vector<int, kallocator<int>> vec = {5, 3, 8, 1, 9};
            ...
            {
                std::vector<int, kallocator<int>> vec = {1, 3, 5, 7, 9, 11};
            ...
            // Algorithms: iota
            {
                std::vector<int, kallocator<int>> vec(10);
            ...
            {
                std::vector<int, kallocator<int>> vec = {1, 2, 3, 4, 5, 6};
            ...
            // Algorithms: merge
            {
                std::vector<int, kallocator<int>> a = {1, 2, 3}, b = {4, 5, 6}, result(6);
            ...
            {
                std::vector<int, kallocator<int>> vec = {10, 3, 7, 1, 5}, result(5);
            ...
            {
                std::vector<int, kallocator<int>> a = {1, 2, 3}, b = {4, 5, 6};
            ...
            }
            {
                ...
                uint32_t v = a.fetch_or(0x000F);
                KTEST_EXPECT(v == 0xFF00 && a.load() == 0xFFF0, "Atomic_FetchOr");
                v = a.fetch_and(0xF0F0);
                KTEST_EXPECT(v == 0xFFF0 && a.load() == 0xF0F0, "Atomic_FetchAnd");
                v = a.fetch_xor(0xF0F0);
                KTEST_EXPECT(v == 0xF0F0 && a.load() == 0, "Atomic_FetchXor");
            }
            {
            ...
            {
                auto a1 = kallocator<int>();
                auto a2 = kallocator<double>();
                KTEST_EXPECT(a1 == a2, "Allocator_Equality");
            }

            // Exception safety
            {
                std::vector<int, kallocator<int>> vec = {1, 2, 3};
    ...
    EXTERN_C VOID DriverUnload(_In_ PDRIVER_OBJECT DriverObject)
    {
        ...
        if (g_TestThread) {
            ...
            KeWaitForSingleObject(g_TestThread, Executive, KernelMode, FALSE, nullptr);
